...
# print u3.couple(SU3State(2,0),SU3State(5,2))

# set1=u3.couple(SU3State(0,1),SU3State(0,1))
# lmdict={}
# for lm1 in set1:
# 	num1=set1[lm1]
# 	set2=u3.couple(lm1,SU3State(2,0))
# 	for lm2 in set2:
# 		num2=set2[lm2]
# 		set3=u3.couple(lm2,SU3State(2,0))
# 		for lm3 in set3:
# 			num3=set3[lm3]
# 			set4=u3.couple(lm3,SU3State(2,0))
# 			for lm4 in set4:
# 				num4=set4[lm4]
# 				set5=u3.couple(lm4,SU3State(2,0))
# 				for lm5 in set5:
# 					if lm5 not in lmdict:
# 						lmdict[lm5]=0
# 					d=num1*num2*num3*num4*set5[lm5]
# 					lmdict[lm5]=lmdict[lm5]+d
# for lm in lmdict:
# 	print lm, lmdict[lm]


#print u3.couple(SU3State(2,2),SU3State(2,0))
# irrep=Sp3RxU2ReducedIrrepN(U3State(28,SU3State(0,2)),0,6)
# for N in irrep:
# 	print N
# 	for state in irrep[N]:
# 		print "   ", state.sp.w
if __name__ == '__main__':
	...
	# for (Z,NN) in ZN:
	# 	print Z, NN
	# 	file1=open("su3cmf/data-120926-{}-{}-SU3xSU2.dat".format(Z,NN),'r')
	# 	for line in file1:
	# 		values=line.split()
	# 		N,Sp,Sn,S,lbda,mu,dimt,dimr=map(int,values)
	# 		S=S/2
	# 		if N not in bandheadsN:
	# 			bandheadsN[N]={}
	# 		s=U3State(N0+N,SU3State(lbda,mu))
	# 		if (s,S) not in bandheadsN[N]:
	# 			bandheadsN[N][s,S]=0
	# 		bandheadsN[N][s,S]=bandheadsN[N][s,S]+dimr
	...
	# 		file2=open("dim_bandheads_{}_{}_{}_{}".format(Z,NN,J,Nbmax),'a')

	# 		dims={}
	# 		for Nb in range(0,Nbmax+2,2):
	# 			for Nex in range(0,Nmax+1,2):
	# 				dims[Nex]=0
	# 				for Ns in range(0,min(Nb,Nex)+1,2):
	# 					for (s,S) in bandheadsN[Ns]:
	# 						dims[Nex]+=len(Sp3RxU2Irrep(s,S,J,Nex-Ns))*bandheadsN[Ns][s,S]
	# 						#print dims[Nex]
	# 				#print Nb, Nex, dims[Nex]
	# 				file2.write("{:4} {:4} {:11}\n".format(Nb,Nex,dims[Nex]))
...
# 	##  Nspace is max excitation for bandheads, Nspace=2 would mean bandheads in the Nex=0 and Nex=2 spaces
 #  ################################################################################################
	# Z=3
	# NN=3
	# bandheads={(0,1):[(1,1)],(2,0):[(0,1)]}
	# #bandheads={(0,1):[(0,1),(1,1)],(2,0):[(0,1),(1,1)]}
	# Ns=11
	################################################################################################
	# Z=4
	# NN=4
	# bandheads={(4,0):[(0,1)],(2,1):[(1,2)],(0,2):[(0,1),(2,1)],(1,0):[(1,4),]}
...
# 	Ns=14.5
	...
	#bandheads={(0,0):[(0,2)],(0,3):[(1,2)],(1,1):[(1,3),(2,2)],(2,2):[(0,1)],(3,0):[(1,1)]}
	#Ns=18
# 	################################################################################################
# 	Z=6
# 	NN=6
...
####################################################################################################################
	Z=8
	NN=10
	bandheads={(0,2):[(0,1)],(2,1):[(1,1)],(4,0):[(0,1)]}
	Ns=28
	...
	for l,m in bandheads:
		...
		for (S,num) in bandheads[l,m]:
			irrepN=Sp3RxU2ReducedIrrepN(s,S,Nmax)
			irrep_dic[s,S,num]=irrepN
	first=1
	for Nex in range(0,Nmax+1,2):
		for s,S,num in irrep_dic:
			last=first+len(irrep_dic[s,S,num][Nex])*(num)
			print "{",s.N+Nex,",", s.su3.lbda,",", s.su3.mu,",",first,",",last,"}"
			first=last

	# #################################################################################################
	# #file1=open('Dimensions/dimJp_{}_{}_{}_{}_{}_red'.format(Z,NN,Nspace,Jmax,Nmax),'a')
	# file2=open('dim-{}-{}_red.dat'.format(Z,NN),'a')
	...
	# 			s=U3State(Ns,SU3State(l,m))
	# 			for (S,num) in bandheads[l,m]:
	# 				dim+=(len(Sp3RxU2ReducedIrrep(s,S,Nex))*num)
	# 		dimNex.append(dim)
