...
int main(int argc, char** argv) {
  if (argc != 6) {
    std::cout << "Usage: " << argv[0] << " <tensor list file> A Nmax N1vp N1vn" << std::endl;
    return EXIT_FAILURE;
  }
  su3::init(250);
/*
...
//---------------------------------------------------
*/
  const int AA = std::stoi(argv[2]);
  const int Nmax = std::stoi(argv[3]);
  const int N1vp = std::stoi(argv[4]);
  const int N1vn = std::stoi(argv[5]);
  ...
  if(dim1!=dim2){
    std::cout<<"dimensions: "<<dim1<<" "<<dim2<<std::endl;
    su3::finalize();
    return EXIT_FAILURE;
  }
...
//    std::cout<<it->second.inverse()<<std::endl;
  ...
  // orthonormal_basis[{gamma,Nex_sigma,Gamma_sigma,SpSnS}][Nex_omega][Gamma_omega][int]
  for(std::map<std::tuple<int,int,SU3,Spins>,std::map<int,std::map<SU3,std::map<std::tuple<SU3,int>,Eigen::VectorXd>>>>::iterator
                  it1=nonorthogonal_basis.begin(); it1!=nonorthogonal_basis.end(); it1++){
  ...
  // Explicit construction of OBDRMEs in SpNCCI basis
  std::map<std::tuple<ONBasisState,ONBasisState,std::array<int,5>>,std::vector<double>> proton_obdrmes,neutron_obdrmes;
/*
  for(const std::array<int,5>& tensor : tensors[0]){
    for(std::map<std::tuple<int,int,SU3,Spins>,std::map<int,std::map<SU3,std::map<int,Eigen::VectorXd>>>>::iterator
      ...
      for(std::map<std::tuple<int,int,SU3,Spins>,std::map<int,std::map<SU3,std::map<int,Eigen::VectorXd>>>>::iterator
                  it1ket=orthonormal_basis.begin(); it1ket!=orthonormal_basis.end(); it1ket++){
	if(std::abs(std::get<3>(it1ket->first).SS()-tensor[4])>std::get<3>(it1bra->first).SS()
	   || std::get<3>(it1bra->first).SS()>std::get<3>(it1ket->first).SS()+tensor[4])continue;
        for(int Nexbra=0; Nexbra<=Nmax; Nexbra=Nexbra+2){
  	  int Nexket=Nexbra-tensor[0]+tensor[1];
	  ...
	  if(Nexket>Nmax)break;
	    ...
	    for(std::map<SU3,std::map<int,Eigen::VectorXd>>::iterator
	      ...
	      int rhomax=su3::mult(it2ket->first.lm(),it2ket->first.mu(),tensor[2],tensor[3],it2bra->first.lm(),it2bra->first.mu());
	      if(rhomax==0)continue;
              for(std::map<int,Eigen::VectorXd>::iterator it3bra=it2bra->second.begin(); it3bra!=it2bra->second.end(); it3bra++){
		...
		for(std::map<int,Eigen::VectorXd>::iterator it3ket=it2ket->second.begin(); it3ket!=it2ket->second.end(); it3ket++){
		  ...
		  for(Ipini ipini_bra : lsu3shell_basis[{std::get<3>(it1bra->first),Nexbra,it2bra->first}]){
		    ...
		    for(Ipini ipini_ket : lsu3shell_basis[{std::get<3>(it1ket->first),Nexket,it2ket->first}]){
		      ...
		      std::map<std::tuple<LSU3shellBasisState,LSU3shellBasisState,std::array<int,5>>,std::vector<double>>::iterator
			      iter=proton_obdrmes_lsu3shell.find({lsu3shell_bra,lsu3shell_ket,tensor});
		      if(iter!=proton_obdrmes_lsu3shell.end()){
			for(int irho=0; irho<rhomax; irho++){
                          proton_rmes[irho]+=it3bra->second[ind_bra]*proton_obdrmes_lsu3shell[{lsu3shell_bra,lsu3shell_ket,tensor}][irho]
                                *it3ket->second[ind_ket];
                        }
		      }
		      iter=neutron_obdrmes_lsu3shell.find({lsu3shell_bra,lsu3shell_ket,tensor});
		      if(iter!=neutron_obdrmes_lsu3shell.end()){
			for(int irho=0; irho<rhomax; irho++){
                          neutron_rmes[irho]+=it3bra->second[ind_bra]*neutron_obdrmes_lsu3shell[{lsu3shell_bra,lsu3shell_ket,tensor}][irho]
                                *it3ket->second[ind_ket];
                        }
		      }
                    }
                  }
                  proton_obdrmes[{bra,ket,tensor}]=proton_rmes;
		  neutron_obdrmes[{bra,ket,tensor}]=neutron_rmes;
                }
              }
	    }
          }
  ...
  std::cout<<"Explicit construction of OBDRMEs in SpNCCI basis finished"<<std::endl;
*/
  // Read explicitly constructed OBDRMEs
  std::ifstream ipfile("explicit_obdrmes_proton.dat");
  if(!ipfile){
    std::cout<<"Could not open explicit_obdrmes_proton.dat!"<<std::endl;
    su3::finalize();
    exit(EXIT_FAILURE);
  }
  while(true){
    ...
    if(ipfile){
      ...
      proton_obdrmes[{bra,ket,tensor}]=rmes;
    }else{
      break;
    }
  }
  std::ifstream infile("explicit_obdrmes_neutron.dat");
  if(!infile){
    std::cout<<"Could not open explicit_obdrmes_neutron.dat!"<<std::endl;
    su3::finalize();
    exit(EXIT_FAILURE);
  }
  while(true){
    ...
    if(infile){
      ...
      neutron_obdrmes[{bra,ket,tensor}]=rmes;
    }else{
      break;
    }
  }
  std::cout<<"Explicitly constructed OBDRMEs read"<<std::endl;

  // Check symmetries of explicitly constructed proton OBDRMEs
  for(std::map<std::tuple<ONBasisState,ONBasisState,std::array<int,5>>,std::vector<double>>::iterator
    ...
    double factor=sqrt(double((std::get<0>(it->first).SpSnS().SS()+1)
		  *su3dim(std::get<0>(it->first).x_omega().lm(),std::get<0>(it->first).x_omega().mu()))
		  /double((std::get<1>(it->first).SpSnS().SS()+1)
		  *su3dim(std::get<1>(it->first).x_omega().lm(),std::get<1>(it->first).x_omega().mu())));
    int phase=std::get<2>(it->first)[1]-std::get<2>(it->first)[0]-std::get<0>(it->first).x_omega().lm()
    ...
    if((phase/2)*2!=phase)factor=-factor;
    int irho=-1;
    for(double rmer : it->second){
      irho++;
      double rhs=factor*rmer;
      double lhs=proton_obdrmes[{std::get<1>(it->first),std::get<0>(it->first),{std::get<2>(it->first)[1],std::get<2>(it->first)[0],
                 std::get<2>(it->first)[3],std::get<2>(it->first)[2],std::get<2>(it->first)[4]}}][irho];
      if(std::abs(lhs-rhs)>1.0e-5){
        std::cout<<"ERROR: "<<lhs<<"!="<<factor<<"*"<<rmer<<"="<<rhs<<std::endl;
        su3::finalize();
        return EXIT_FAILURE;
      }
    }
  }
  // Check symmetries of explicitly constructed neutron OBDRMEs
  for(std::map<std::tuple<ONBasisState,ONBasisState,std::array<int,5>>,std::vector<double>>::iterator
    ...
    for(double rmer : it->second){
      irho++;
      double rhs=factor*rmer;
      double lhs=neutron_obdrmes[{std::get<1>(it->first),std::get<0>(it->first),{std::get<2>(it->first)[1],std::get<2>(it->first)[0],
                 std::get<2>(it->first)[3],std::get<2>(it->first)[2],std::get<2>(it->first)[4]}}][irho];
      if(std::abs(lhs-rhs)>1.0e-5){
        std::cout<<"ERROR: "<<lhs<<"!="<<factor<<"*"<<rmer<<"="<<rhs<<std::endl;
        su3::finalize();
  ...
  std::cout<<"Symmetries of explicitly constructed OBDRMEs checked"<<std::endl;
/*
  // Output explicitly constructed OBDRMEs
  std::ofstream pfile("explicit_obdrmes_proton.dat");
  if (!pfile) {
    std::cout << "Could not open file explicit_obdrmes_proton.dat" << std::endl;
    return EXIT_FAILURE;
  }
  for(std::map<std::tuple<ONBasisState,ONBasisState,std::array<int,5>>,std::vector<double>>::iterator
                  it=proton_obdrmes.begin(); it!=proton_obdrmes.end(); it++){
    pfile<<std::get<0>(it->first).gamma()<<" "
	  ...
	  <<std::get<2>(it->first)[0]<<" "<<std::get<2>(it->first)[1]<<" "<<std::get<2>(it->first)[2]<<" "
          <<std::get<2>(it->first)[3]<<" "<<std::get<2>(it->first)[4]<<" "<<it->second.size();
    for(double rme : it->second){
      pfile<<" "<<rme;
    }
    pfile<<std::endl;
  }
  std::ofstream nfile("explicit_obdrmes_neutron.dat");
  if (!nfile) {
    std::cout << "Could not open file explicit_obdrmes_neutron.dat" << std::endl;
    return EXIT_FAILURE;
  }
  for(std::map<std::tuple<ONBasisState,ONBasisState,std::array<int,5>>,std::vector<double>>::iterator
                  it=neutron_obdrmes.begin(); it!=neutron_obdrmes.end(); it++){
