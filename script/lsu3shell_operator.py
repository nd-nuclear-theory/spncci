...
## recoupler_executable = spncci.get_lsu3shell_executable("RecoupleSU3Operator")
...
# TODO (mac): update paths to proper installation directory...
## generate_lsu3shell_relative_operators_executable = spncci.get_spncci_executable("generate_lsu3shell_relative_operators")
...
def recouple_operators(task,relative_operator_basename_list):
    """Invoke lsu3shell recoupler code on relative unit tensors and symplectic
    raising/lowering/N operators, as generated in terms of biquads by
    ...
    for basename in relative_operator_basename_list:
        ...
        ## In case restarting a run:
        ##     Search for .PN and .PPNN files for given base name.  
        ##     If found in current working directory, continue to next base name.
        ##     Else, invoke recoupler.
...
def split_relative_operator_file(task):
    """Split relative_operators.dat into two control files, one for generators and
    one for unit tensors.
    """
