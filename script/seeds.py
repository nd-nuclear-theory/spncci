"""seeds.py -- define scripting for spncci seed runs
  ...
  7/26/21 (aem) : Separate out lgi construction from seed calculation.
"""
...
generate_lgi_expansion_executable = os.path.join(project_root,"spncci","programs","lgi","get_lgi_expansion")
...
####################################################################################
def get_lgi_expansion(task):
    """
    Generate the lgi expansion for calculating seed RMEs from su3shell RMEs
    """
    # retrieve relevant operator files
    lsu3.retrieve_generator_operator_files(task)
    mcscript.call(["ln","-sf","relative_generator_operators.dat","relative_operators.dat"])

    # Do any necessary set up for temporary su3shell files  
    su3rme.setup_su3shell_directories(task)
    
    ## Get model space file for calculating Brel and Nrel RMEs in su3shell basis
    ## and create symlink to lgi model space files  files 
    su3rme.generate_model_space_files_lgi(task)
    mcscript.call(["ln","-sf","lgi_model_space_bra.dat","model_space_bra.dat"])
    mcscript.call(["ln","-sf","lgi_model_space_ket.dat","model_space_ket.dat"])
    
    ## Calculate the Brel and Nrel RMEs
    su3rme.calculate_rmes(task)
    
    ## Generate basis listing 
    su3rme.generate_basis_table("model_space_bra.dat","lsu3shell_basis_bra.dat",task)
    su3rme.generate_basis_table("model_space_ket.dat","lsu3shell_basis_ket.dat",task)
