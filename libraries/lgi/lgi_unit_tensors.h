...
//*********************************************** Added by J.H. ********************************************
  typedef std::tuple<u3shell::OneBodyUnitTensorLabelsU3S,int,int,int> OneBodySeedLabels;
  typedef std::unordered_map<std::pair<int,int>,std::vector<OneBodySeedLabels>,boost::hash<std::pair<int,int>>>
    LGIGroupedOneBodySeedLabels;
  ...
  void ComputeOneBodyUnitTensorSeedBlocks(
    ...
    // Compute seed blocks for SpNCCI recurrence for one-body unit tensor RMEs from LSU3Shell rmes.
    // The seeds are stored in a matrix array by unit tensor then by unit tensor lgi sector index.
    //
    // Corresponding to the seed blocks is a nested unordered map of 
    // {lgi_pair: {unit_tensor_labels: (rho0,unit_tensor_index,sector_index)}
    // Only those sectors for lgi_bra>=lgi_ket and which have non-zero rmes are included.
  ...
  void WriteOneBodySeedsToFile(
    ...
    // For each LGI pair (bra>=ket), write seed rmes to file 
    //  fmt::format("obseeds_{:06d}_{:06d}.rmes",lgi_bra_index,lgi_ket_index)
    //
    // and creates corresponding list of unit tensor labels in 
    //  fmt::format("oboperators_{:06d}_{:06d}.dat",lgi_bra_index,lgi_ket_index).
    //
    // In the seed file, the rmes are 
    //    by unit tensor (order given by unit_tensor_file)
    //      by rho0
    //
    // Unit tensor file:
    //    lambda0 mu0 S0 etap eta num_sectors(number of non-zero sectors ~ rho0max)
    //
    // seed file:
    //    rho0, rows, columns, rmes....

  typedef std::tuple<u3shell::TwoBodyDensityLabels,int,int,int> TwoBodySeedLabels;
  typedef std::unordered_map<std::pair<int,int>,std::vector<TwoBodySeedLabels>,boost::hash<std::pair<int,int>>>
    LGIGroupedTwoBodySeedLabels;
  ...
  void ComputeTwoBodyDensitySeedBlocks(
    ...
    // Compute seed blocks for SpNCCI recurrence for two-body density RMEs from LSU3Shell rmes.
    // 
    // Only those sectors for lgi_bra>=lgi_ket and which have non-zero rmes are included.
  ...
  void WriteTwoBodySeedsToFile(
    ...
    // For each LGI pair (bra>=ket), write seed rmes to file
    //  fmt::format("tbseeds_{:06d}_{:06d}.rmes",lgi_bra_index,lgi_ket_index)
    //
    // and creates corresponding list of two-body density labels in
    //  fmt::format("tboperators_{:06d}_{:06d}.dat",lgi_bra_index,lgi_ket_index).
    //
    // In the seed file, the rmes are
    //    by operator (order given by operator file)
    //      by rho
    //
    // Operator file:
    //    lm0 mu0 SS0 N1 N2 N3 N4 lmf muf SSf lmi mui SSi rho0 num_sectors(number of non-zero sectors ~ rhomax)
    //
    // seed file:
    //    rho, rows, columns, rmes....
//**********************************************************************************************************
...
//******************************************** Added by J.H. *******************************************
  bool ReadOneBodyUnitTensorLabels(
    ...
    // Reads in one-body unit tensor labels from file and stores them in a pair of vectors.  One 
    // for the unit tensor labels and one for the corresponding rho0
    //  Labels in file are:
    //    lambda0, mu0, 2S0, etap, eta, Tz, rho0
    //
    // If file found and sucessfully opened, returns file_found=true.  Otherwise,
    // function returns file_found=false.
  ...
  bool ReadTwoBodyDensityLabels(
    ...
    // Reads in two-body density labels from file and stores them in a pair of vectors.  One
    // for the TBD labels and one for the corresponding rho
    //  Labels in file are:
