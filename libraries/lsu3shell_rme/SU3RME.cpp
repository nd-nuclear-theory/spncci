...
double ComputeRME(std::vector<MECalculatorData>& rmeCoeffsPNPN, int braSSp, int braSSn, int braSS, int bralm, int bramu)
{
//	Test if rho0max == 1	
	for (size_t itensor = 0; itensor < rmeCoeffsPNPN.size(); ++itensor)
	{
		if (rmeCoeffsPNPN[itensor].rmes_->m_ntotal > 1)
		{
			std::cerr << "A given tensor or bra/ket irreps have multiplicities > 1.";
			std::cerr << "Given computation is implemented only for case when afmax == aimax == rh0max == rhotmax == 1. We need to generalize our algorithm first!" << std::endl;
			exit(EXIT_FAILURE);
		}
	}
	...
	for (size_t itensor = 0; itensor < rmeCoeffsPNPN.size(); ++itensor)
	{
		rme += rmeCoeffsPNPN[itensor].rmes_->m_rme[0]*rmeCoeffsPNPN[itensor].coeffs_[0];
	}
					...
					const lsu3::CncsmSU3xSU2Basis& ket) 
{
	...
	{
//		We don't have to check whether bra.NumberOfStatesInBlock(ipin_block) > 0 since we are interested
...
//	Generally, ket space can be different from bra space. For this reason we loop over all ket jpjn blocks.
		for (unsigned int jpjn_block = 0; jpjn_block < number_jpjn_blocks; jpjn_block++)
		{
			...
			for (int32_t iwpn = ibegin; iwpn < iend; ++iwpn)
			{
				...
				for (int32_t jwpn = jbegin; jwpn < jend; ++jwpn)
				{
...
//	calculate <	{(lmp mup)Sp x (lmn mun)Sn} wf Sf ||| [T_{nn} x I_{pp}] |||{(lmp' mup')Sp' x (lmn' mun')Sn'} wi Si >_{rhot}		 	
						...
						{
							double rme = ComputeRME(rmeCoeffsPNPN, w_ip.S2, w_in.S2, omega_pn_I.S2, omega_pn_I.lm, omega_pn_I.mu);
							cout << "(" << ip << ", " << in << ") ";
							cout << (int) omega_pn_I.rho << "(" << (int)omega_pn_I.lm << " " << (int)omega_pn_I.mu << ")" << (int)omega_pn_I.S2;
							cout << "|| O ||";
							cout << "(" << jp << ", " << jn << ") ";
							cout << (int) omega_pn_J.rho << "(" << (int)omega_pn_J.lm << " " << (int)omega_pn_J.mu << ")" << (int)omega_pn_J.S2;
							cout << " rme:" << rme << endl;
						}
...
void CalculateRMEs(const std::string& bra_space_definition_file_name, const std::string& ket_space_definition_file_name, const std::string& hamiltonian_file_name)
{
	int idiag = 0;
	int jdiag = 0;
	int ndiag = 1;
...
//	if rme file does not exist, then false
	...
	run_params.LoadHamiltonian(0, hamiltonian_file_name, interactionPPNN, interactionPN, bra.NProtons()+bra.NNeutrons());
...
//	these two variable are needed for MFDn
	CalculateRMEs(bra_space_definition_file_name, ket_space_definition_file_name, hamiltonian_file_name);
