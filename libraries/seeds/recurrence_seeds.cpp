...
{
using LGILookupTable =
    std::unordered_map<u3::U3, std::vector<std::pair<int, lgi::LGI>>, boost::hash<u3::U3>>;
...
// Retruns a map keyed by sigma with value (irrep_family_index,lgi)
{
  ...
  auto extract_sigma =
      [](const auto& enum_lgi) { return enum_lgi.element.irrep.U3(); };
  auto compare_sigma =
      [&extract_sigma](
          const auto& enum_lgi1, const auto& enum_lgi2
        ) { return extract_sigma(enum_lgi1) < extract_sigma(enum_lgi2); };
  auto regrouped_lgi_vector = iter::groupby(
      iter::sorted(iter::enumerate(lgi_vector), compare_sigma), extract_sigma
    );
...
//
///////////////////////////////////////////////////////////////////////////////////////
basis::OperatorBlock<double> ReadSeedBlockFromFile(
    const LGILookupTable& lgi_groups,
    const std::vector<int>& lgi_full_space_index_lookup,
    const spncci::spatial::RecurrenceU3Space<u3shell::spatial::OneCoordType>&
        lgi_spatial_space,
  ...
  )
{
  ...
  seed_block = basis::OperatorBlock<double>::Zero(
      lgi_spatial_space.dimension(), lgi_spin_space.dimension()
    );
  ...
  for (const auto& [irrep_family_index_bra, lgi_bra] : lgi_groups.at(sigma_bra))
    for (const auto& [irrep_family_index_ket, lgi_ket] : lgi_groups.at(sigma_ket))
    {
      ...
      const int degeneracy_ket =
          spin_space.GetKetSubspaceDegeneracy(spin_subspace_index);
      const int degeneracy_bra =
          spin_space.GetBraSubspaceDegeneracy(spin_subspace_index);
      ...
      // Corresponding rho0 values stored separately for later hypersector
      // lookup but could just be recalculated.
      ///////////////////////////////////////////////////////////////////////////
      ...
      lgi::ReadUnitTensorLabels(
          lgi_unit_tensor_filename, lgi_unit_tensors, rho0_values
        );
      ...
      lgi::ReadBlocks(
          seed_filename, lgi_unit_tensors.size(), unit_tensor_seed_blocks
        );
      ...
      for (int t = 0; t < lgi_unit_tensors.size(); ++t)
      {
        ...
        // Look up spin operator.  Note that cast to int necessary because unit
        // tensor spin labels are currently HalfInt type even though they are
...
)
{
  ...
  for (int sp3r_space_index = 0;
       sp3r_space_index < spatial_recurrence_space.size();
       ++sp3r_space_index)
  {
    const auto& recurrence_sp3r_space =
        spatial_recurrence_space.GetSubspace(sp3r_space_index);
    ...
    const auto& lgi_spatial_space =
        recurrence_sp3r_space.GetSubspace(0).GetSubspace(0);

    // To enforce antisymmetry
    uint8_t exchange_symmetry = (parity_bar + 1) % 2;

    // Get corresponding spin space
    const auto& lgi_spin_space = spin_recurrence_space.LookUpSubspace(
        {sigma_ket, sigma_bra, exchange_symmetry}
      );
