...
unsigned int OuterMultiplicity(const u3::SU3& x1, const u3::SU3& x2, const u3::SU3& x3)
{
  ...
  int Nx = int(x1.lambda_u() + x2.lambda_u() + x3.mu_u())
           - int(x1.mu_u() + x2.mu_u() + x3.lambda_u());
  ...
  if (Mu >= 0)
  {
    ...
    if (Nu >= 0)
      multiplicity = static_cast<unsigned int>(
          std::max(std::min(Nx, Nu) - std::max(Nx - Mu, 0) + 1, 0)
        );
  }
  ...
  )
{
  ...
  unsigned int lambda3_max = x1.lambda_u() + x2.lambda_u() + std::min(x2.mu_u(), x1.lambda_u() + x1.mu_u());
  unsigned int mu3_max = x1.mu_u() + x2.mu_u() + std::min(x1.lambda_u(), x2.lambda_u());

  // allocate container for product
  MultiplicityTagged<u3::SU3>::vector product;
  std::size_t max_entries = static_cast<std::size_t>(
      (lambda3_max - lambda3_min + 1) * (mu3_max - mu3_min + 1)
    );
  ...
  )
{
  ...
  // couple SU(3)
  MultiplicityTagged<u3::SU3>::vector su3_product =
      KroneckerProduct(omega1.SU3(), omega2.SU3());
...
unsigned int BranchingMultiplicitySO3(const u3::SU3& x, unsigned int L)
{
  ...
  int Lint = static_cast<int>(L);
  unsigned int multiplicity = static_cast<unsigned int>(
      std::max(0, (x.lambda() + x.mu() + 2 - Lint) / 2)
      - std::max(0, (x.lambda() + 1 - Lint) / 2)
      - std::max(0, (x.mu() + 1 - Lint) / 2)
    );
  return multiplicity;
}
...
MultiplicityTagged<unsigned int>::vector BranchingSO3(const u3::SU3& x)
{
  ...
  unsigned int L_min = std::min(x.lambda_u(), x.mu_u()) % 2;
  unsigned int L_max = x.lambda_u() + x.mu_u();

  // allocate container for product
  MultiplicityTagged<unsigned int>::vector branching;
  // int max_entries = static_cast<std::size_t>(L_max - L_min + 1);
  branching.reserve(static_cast<std::size_t>(L_max - L_min + 1));
  ...
  )
{
  ...
  // allocate container for product
  MultiplicityTagged<unsigned int>::vector branching;
  int max_entries = std::max(
      L_max - L_min + 1, 0
    );  // expression might be negative so impose floor of 0
