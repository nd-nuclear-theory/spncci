...
std::vector<u3::U3> RaisingPolynomialLabels(unsigned int Nn_max)
{
  ...
  for (int N = 2; N <= Nn_max; N += 2)
    for (int a = N - 2; a >= 0; a -= 2)
      for (int b = 2 * (a / 4); b >= std::max((2 * a - N), 0); b -= 2) //2*integer division of a/4
        poly_labels.push_back(u3::U3(N - a, a - b, b));

  return poly_labels;
}

U3BosonSpace::U3BosonSpace(const u3::U3& sigma, const unsigned int Nn_max)
...
U3BosonSectors::U3BosonSectors(const U3BosonSpace& space, const u3::U3& omega0)
...
{
  double rme = 0;
  const int n1 = static_cast<int>(n.f1());
  const int n2 = static_cast<int>(n.f2());
  const int n3 = static_cast<int>(n.f3());
  const int n1p = static_cast<int>(np.f1());
  const int n2p = static_cast<int>(np.f2());
  const int n3p = static_cast<int>(np.f3());
  ...
  if ((n1p == (n1 + 2)) && (n2p == n2) && (n3p == n3))
    ...
    rme = std::sqrt(
        (n2 + 3) * (n1 - n2) * (n2 - n3 + 2) / (2. * (n1 - n2 - 1) * (n2 - n3 + 3))
      );
...
double U3BosonCreationRME(
    const u3::U3& sigmap,
    const MultiplicityTagged<u3::U3> np_rhop,
    const u3::U3& omegap,
    const u3::U3& sigma,
    const MultiplicityTagged<u3::U3> n_rho,
    const u3::U3& omega
  )
{
...
basis::OperatorBlock<double> U3BosonCreationOperator(
    const u3::U3& sigma,
  ...
  )
{
  ...
  operator_block = basis::OperatorBlock<double>::Zero(
      bra_subspace.dimension(), ket_subspace.dimension()
    );

  for (std::size_t bra_state_index = 0; bra_state_index < bra_subspace.size();
       ++bra_state_index)
    for (std::size_t ket_state_index = 0; ket_state_index < ket_subspace.size();
         ++ket_state_index)
    {
      ...
      for (unsigned int rho_bra = 1; rho_bra <= rho_max_bra; ++rho_bra)
        for (unsigned int rho_ket = 1; rho_ket <= rho_max_ket; ++rho_ket)
        {
          ...
          operator_block(row, col) = U3BosonCreationRME(
              sigma, n_bra, rho_bra, omega_bra, sigma, n_ket, rho_ket, omega_ket
            );
        }
    }
...
basis::OperatorBlock<double> U3BosonAnnihilationOperator(
    const u3::U3& sigma,
    const u3boson::U3Subspace& bra_subspace,
    const u3boson::U3Subspace& ket_subspace,
    u3::UCoefCache& u_coef_cache
  )
{
  ...
  return conjugation_factor
         * U3BosonCreationOperator(sigma, ket_subspace, bra_subspace, u_coef_cache).transpose();
}
