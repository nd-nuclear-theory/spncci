/****************************************************************
  ...
  9/6/16 (mac): Upgrade U3S and U3ST from struct to class with hash function,
etc. 09/24/21 (pjf):
...
class SU3
{
 ...
 public:
  ...
  constexpr inline int lambda() const { return int(lambda_); }

  constexpr inline int mu() const { return int(mu_); }

  constexpr inline unsigned int lambda_u() const { return lambda_; }

  constexpr inline unsigned int mu_u() const { return mu_; }

  ////////////////////////////////////////////////////////////////
  // key tuple, comparisons, and hashing
  ////////////////////////////////////////////////////////////////

  using KeyType = std::pair<unsigned int, unsigned int>;

  constexpr inline KeyType Key() const { return KeyType(lambda(), mu()); }
...
{
  return 2. / 3
         * ((x.lambda() * x.lambda()) + x.lambda() * x.mu() + (x.mu() * x.mu())
            + 3 * x.lambda() + 3 * x.mu());
}
...
// Third order Casimir
{
  return 1. / 9 * (x.lambda() - x.mu()) * (x.lambda() + 2 * x.mu() + 3)
         * (2 * x.lambda() + x.mu() + 3);
}
...
class U3
{
 ...
 public:
  ...
  // validation
  ////////////////////////////////////////////////////////////////
  constexpr inline static bool ValidLabels(
      const HalfInt& f1, const HalfInt& f2, const HalfInt& f3
    )
  // Check validity of U3 labels in Cartesian form.
  {
    return ((f1 >= f2) && (f2 >= f3) && ((f3 >= 0) || (f1 <= 0)))
           && IsInteger(f1 - f2) && IsInteger(f2 - f3);
  }


  constexpr inline static bool ValidLabels(const HalfInt& N, const u3::SU3& x)
  // Check validity of U3 labels in N(lambda,mu) form.
  {
    // bool valid = TwiceValue(N - x.lambda() + 2 * x.mu()) % 3 == 0;
    bool valid = TwiceValue(N - x.lambda() - 2 * x.mu()) % 3 == 0;
  ...
  constexpr inline KeyType Key() const { return KeyType(N_, x_); }
...
//
// Reference: C. K. Chew and R. T. Sharp, Can. J. Phys. 44, 2789 (1966).  To be
// verified. As in SU3LIB MULTU3 or UNU3SU3Basics SU3::mult.
//
// Arguments:
...
MultiplicityTagged<u3::U3>::vector KroneckerProduct(
    const u3::U3& omega1, const u3::U3& omega2
  );
// Overloaded for U3.
...
MultiplicityTagged<unsigned int>::vector BranchingSO3Constrained(
    const u3::SU3& x, const HalfInt::pair& r
  );
// DEPRECATED
...
// result of coupling J and S using am::ProductAngularMomentumRange, the actual
...
template<> struct formatter<u3::SU3>
{
  char presentation = 'g';
...
template<> struct formatter<u3::U3>
{
  char presentation = 'g';
  ...
  template<typename FormatContext>
  FMT_CONSTEXPR auto format(const u3::U3& w, FormatContext& ctx)
      -> decltype(ctx.out())
  {
    if (presentation == 'f')
