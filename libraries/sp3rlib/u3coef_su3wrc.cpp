...
extern "C"
{
  ...
  extern void
  wru3optimized_(const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, double[], const int&);
  extern void
  wzu3optimized_(const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, double[], const int&);
  extern void
  wu39lm_(const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, const int&, double[], const int&);
...
WCoefBlock::WCoefBlock(const u3::WCoefLabels& labels)
{
  ...
  int size = static_cast<int>(rho_max_ * kappa1_max_ * kappa2_max_ * kappa3_max_);
  coefs_.reserve(size);

  // coefs are in column-major Fortran array
  for (unsigned int rho = 1; rho <= rho_max_; ++rho)
    for (unsigned int kappa2 = 1; kappa2 <= kappa2_max_; ++kappa2)
      for (unsigned int kappa1 = 1; kappa1 <= kappa1_max_; ++kappa1)
        for (unsigned int kappa3 = 1; kappa3 <= kappa3_max_; ++kappa3)
  ...
  )
{
  ...
  // pre-size vector
  int r_max = static_cast<int>(r12_max_ * r12_3_max_ * r23_max_ * r1_23_max_);
  assert(r_max > 0);
  coefs_.resize(r_max);
...
PhiCoefBlock::PhiCoefBlock(const u3::PhiCoefLabels& labels)
{
  ...
  // zero initialize array
  int rho_dummy = 1;
  int dim = static_cast<int>(rho_max_ * rho_max_);
  coefs_.resize(dim);
  su3lib::wzu3optimized_(
      x1.lambda(),
      x1.mu(),
...
{
  unsigned int r12_max = u3::OuterMultiplicity(x1, x2, x12);
  unsigned int r34_max = u3::OuterMultiplicity(x3, x4, x34);
  unsigned int r13_max = u3::OuterMultiplicity(x1, x3, x13);
  unsigned int r24_max = u3::OuterMultiplicity(x2, x4, x24);
  unsigned int r13_24_max = u3::OuterMultiplicity(x13, x24, x);
  unsigned int r12_34_max = u3::OuterMultiplicity(x12, x34, x);
  std::size_t index =
      (r12_max * (r34 - 1) + r12_max * r34_max * (r12_34_max - 1)
  ...
  int r_max = static_cast<int>(r12_max * r34_max * r13_max * r24_max * r12_34_max * r13_24_max);
