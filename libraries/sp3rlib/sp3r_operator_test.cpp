...
namespace sp3r{
  ...
  )
  {
    ...
    if(omega_bra!=omega_ket)
      return sp3r::Matrix::Zero(bra_subspace.dimension(),ket_subspace.dimension());

    sp3r::Matrix matrix
      = std::sqrt(2./3)*TwiceValue(omega_bra.N())
          *sp3r::Matrix::Identity(bra_subspace.dimension(),ket_subspace.dimension());
        ...
        for(const auto& ket_subspace : sp3r_space)
          {
            ...
            // fmt::print("({}||{}||{})\n",omega_bra,w0,omega_ket);
            sp3r::Matrix commutator_matrix = sp3r::Matrix::Zero(bra_subspace.dimension(),ket_subspace.dimension());
