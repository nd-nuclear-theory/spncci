/****************************************************************
  ...
  5/4/22 (aem) : Changed all multiplicities and L values to unsigned int
****************************************************************/
#ifndef U3COEF_H_
#define U3COEF_H_
...
// Class to gather and provide hashing for U coefficient labels
{
 public:
  ...
  using KeyType =
      std::tuple<u3::SU3, unsigned int, u3::SU3, unsigned int, u3::SU3, unsigned int>;
  ...
  inline KeyType Key() const { return KeyType(x1_, L1_, x2_, L2_, x3_, L3_); }
 ...
 private:
  // Operator labels
  u3::SU3 x1_, x2_, x3_;
  unsigned int L1_, L2_, L3_;
};


using WMultiplicityTuple =
  ...
  )
{
  ...
  unsigned int kappa1_max = u3::BranchingMultiplicitySO3(x1, L1);
  unsigned int kappa2_max = u3::BranchingMultiplicitySO3(x2, L2);
  unsigned int kappa3_max = u3::BranchingMultiplicitySO3(x3, L3);
...
// kappa1_max,kappa2_max, kappa3_max,rho_max
{
 public:
    ...
    ) const
  {
    ...
    assert(
        (rho <= rho_max_) && (kappa1 <= kappa1_max_) && (kappa2 <= kappa2_max_)
        && (kappa3 <= kappa3_max_)
      );

    // calculate index into block
    std::size_t index = CoefIndex(kappa1, kappa2, kappa3, rho);
    return coefs_[index];
  }

  inline std::vector<double> GetCoefBlock() const { return coefs_; }

  std::size_t CoefIndex(
      const unsigned int kappa1,
      const unsigned int kappa2,
      const unsigned int kappa3,
      const unsigned int rho
 ...
 private:
  // multiplicities
  unsigned int kappa1_max_, kappa2_max_, kappa3_max_, rho_max_;
...
////////////////////////////////////////////////////////////////
using WCoefCache =
    std::unordered_map<u3::WCoefLabels, u3::WCoefBlock, boost::hash<u3::WCoefLabels>>;
...
using UMultiplicityTuple =
    std::tuple<unsigned int, unsigned int, unsigned int, unsigned int>;

inline u3::UMultiplicityTuple UMultiplicity(
    const u3::SU3& x1,
    const u3::SU3& x2,
    const u3::SU3& x,
    const u3::SU3& x3,
...
{
  unsigned int rho12_max = u3::OuterMultiplicity(x1, x2, x12);
  unsigned int rho12_3_max = u3::OuterMultiplicity(x12, x3, x);
  unsigned int rho23_max = u3::OuterMultiplicity(x2, x3, x23);
  unsigned int rho1_23_max = u3::OuterMultiplicity(x1, x23, x);
...
// Class to gather and provide hashing for U coefficient labels
{
 public:
  ...
  inline KeyType Key() const { return KeyType(x1_, x2_, x_, x3_, x12_, x23_); }
  ...
  {
    unsigned int rho12_max, rho12_3_max, rho23_max, rho1_23_max;
    std::tie(rho12_max, rho12_3_max, rho23_max, rho1_23_max) =
        UMultiplicity(x1_, x2_, x_, x3_, x12_, x23_);

    unsigned int rho_max = rho12_max * rho12_3_max * rho23_max * rho1_23_max;
...
// Choice of RecoupingMode determines if U and Z coefficient computed
{
 public:
    ...
    ) const
  {
    // validate multiplicity indices
    assert(
        (r12 <= r12_max_) && (r12_3 <= r12_3_max_) && (r23 <= r23_max_)
        && (r1_23 <= r1_23_max_)
      );
    std::size_t index = CoefIndex(r12, r12_3, r23, r1_23);
    return coefs_[index];
  }
 ...
 private:
  ...
  ////////////////////////////////////////////////////////////////
  // multiplicities
  unsigned int r12_max_, r12_3_max_, r23_max_, r1_23_max_;
  // Recoupling mode
...
using UCoefCache =
    std::unordered_map<u3::UCoefLabels, u3::RecouplingCoefBlock, boost::hash<u3::UCoefLabels>>;
...
inline double U(
  ...
  unsigned int r1_23)
{
  auto block = RecouplingCoefBlock(
      {SU3(x1), SU3(x2), SU3(x), SU3(x3), SU3(x12), SU3(x23)}, RecouplingMode::kU
    );
...
// Class to gather and provide hashing for U coefficient labels
{
 public:
  ...
  inline KeyType Key() const { return KeyType(x1_, x2_, x3_); }
  ////////////////////////////////////////////////////////////////
  // hashing
  ////////////////////////////////////////////////////////////////
...
//
{
 public:
  ...
  inline double GetCoef(unsigned int rho1, unsigned int rho2) const
  {
    // validate multiplicity indices
    assert((rho1 <= rho_max_) && (rho2 <= rho_max_));
    assert(rho2 > 0 && rho1 > 0);
    std::size_t index = static_cast<std::size_t>(rho2 - 1);
...
using PhiCoefCache =
    std::unordered_map<u3::PhiCoefLabels, u3::PhiCoefBlock, boost::hash<u3::PhiCoefLabels>>;
