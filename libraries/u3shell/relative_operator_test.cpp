...
int main(int argc, char** argv)
{
  ...
  if (true)
  {
    ...
    auto quadrupoleT1_parameters =
        u3shell::relative::QIsovectorParameters(Nbar_max);
    ...
    u3shell::GenerateRelativeUnitTensorLabelsU3ST(
        Nmax, N1v, relative_unit_tensor_labels
      );
    ...
    for (const auto& tensor : relative_unit_tensor_labels)
    {
      ...
      Allowed_S0_values.insert(
          int(S0)
        );  // No direct conversion from HaltInt to uint8_t
    ...
    u3shell::spin::twobody::OperatorSpace spin_operator_space(
    ...
    u3shell::relative::OperatorSectors operator_sectors =
        u3shell::relative::ConstructOperatorSectors(unit_tensor_parameters);
  ...
  if (true)
  {
    ...
    // Quadrupole operator
    ////////////////////////////////////////////////////////////////
    auto quadrupoleT0_parameters =
        u3shell::relative::QIsoscalarParameters(Nbar_max);
    auto quadrupoleT1_parameters =
        u3shell::relative::QIsovectorParameters(Nbar_max);

    auto sectors_T0 =
        u3shell::relative::ConstructOperatorSectors(quadrupoleT0_parameters);
    auto sectors_T1 =
        u3shell::relative::ConstructOperatorSectors(quadrupoleT1_parameters);

    std::vector<double> quadrupole_test0 =
        u3shell::relative::RelativeOperatorRMEs(
            sectors_T0, u3shell::relative::QuadrupoleRME
          );
    std::vector<double> quadrupole_test1 =
        u3shell::relative::RelativeOperatorRMEs(
            sectors_T1, u3shell::relative::QuadrupoleRME
          );

    u3shell::relative::RelativeOperator quadruple_operator_T0(
        quadrupoleT0_parameters, u3shell::relative::QuadrupoleRME
      );

    u3shell::relative::RelativeOperator quadruple_operator_T1(
        quadrupoleT1_parameters, u3shell::relative::QuadrupoleRME
      );

    // Check that the stored rmes are the same.
    for (std::size_t i = 0; i < quadrupole_test0.size(); ++i)
    {
      assert(fabs(quadrupole_test0[i] - quadruple_operator_T0.rmes()[i]) < 1e-10);
    }

    for (std::size_t i = 0; i < quadrupole_test1.size(); ++i)
    {
    ...
    auto combined_parameters = u3shell::relative::CombineParameters(
        {quadrupoleT0_parameters, quadrupoleT1_parameters}
      );

    auto sectors =
        u3shell::relative::ConstructOperatorSectors(combined_parameters);
    std::vector<double> quadrupole_test = u3shell::relative::RelativeOperatorRMEs(
        sectors, u3shell::relative::QuadrupoleRME
      );

    u3shell::relative::RelativeOperator quadruple_operator(
        combined_parameters, u3shell::relative::QuadrupoleRME
      );

    for (std::size_t i = 0; i < quadrupole_test.size(); ++i)
    {
    ...
    unsigned int Nmax = 40;
    unsigned int Jmax = Nmax + 1;
    u3shell::relative::RelativeOperator daejeon_operator(
        Nbar_max,
        u3shell::relative::HamiltonianParameters(Nbar_max),
        inputfile,
        Nmax,
        Jmax
      );
    ...
    basis::ReadRelativeOperatorLSJT(
        inputfile, relative_space_lsjt, op_labels_lsjt, sectors_lsjt, blocks_lsjt, true
      );

    // For testing
    u3shell::RelativeRMEsU3ST rme_map;
    u3shell::Upcoupling(
        relative_space_lsjt, sectors_lsjt, blocks_lsjt, 0, 0, -1, Nbar_max, rme_map
      );

    for (const auto& [key, rme1] : rme_map)
    {
      const auto& [labels, kappa0, L0] = key;
      const auto& [x0, S0, T0, Nbarp, Sbarp, Tbarp, Nbar, Sbar, Tbar] =
          labels.FlatKey();
      ...
      double rme2 = daejeon_operator.ReducedMatrixElement(
          {x0, int(S0), int(T0)}, bra, ket, kappa0, L0
        );
      // fmt::print("{} {} {}   {} {} {}   {} {} {}  {}
      // {}\n",x0,S0,T0,Nbarp,Sbarp,Tbarp,Nbar,Sbar,Tbar,kappa0,L0); std::cout<<rme1<<" "<<rme2<<std::endl;
    ...
    for (std::size_t i = 0; i < rmes1.size(); ++i)
    {
    ...
    ////////////////////////////////////////////////////////////////
    auto hamiltonian_parameters = u3shell::relative::CombineParameters(
        {u3shell::relative::KSquaredParameters(Nbar_max),
         u3shell::relative::HamiltonianParameters(Nbar_max, 0u, 0u)}
      );

    auto hamiltonian_sectors =
        u3shell::relative::ConstructOperatorSectors(hamiltonian_parameters);
    ...
    u3shell::relative::RelativeOperator hamiltonian_operator(
        hamiltonian_parameters, hamiltonian_sectors, hamiltonian_rmes
      );
  }
