  ...
  )
{
  ...
  const int Jbar_min = abs(static_cast<int>(Lbar-Sbar));
  const int Jbarp_min = abs(static_cast<int>(Lbarp-Sbarp));

  std::size_t bra_index = bra_space_lsjt.LookUpSubspaceIndex(
      {Lbarp, Sbarp, Jbarp_min, Tbarp, Lbarp % 2}
    );
  std::size_t ket_index =
      ket_space_lsjt.LookUpSubspaceIndex({Lbar, Sbar, Jbar_min, Tbar, Lbar % 2});
  ...
  for (int Jbar = Jbar_min; Jbar <= Lbar + Sbar; ++Jbar)
    for (int Jbarp = Jbarp_min; Jbarp <= Lbarp + Sbarp; ++Jbarp)
    {
      ...
      size_t bra_index = bra_space_lsjt.LookUpSubspaceIndex(
          {Lbarp, Sbarp, Jbarp, Tbarp, Lbarp % 2}
        );
      size_t ket_index =
          ket_space_lsjt.LookUpSubspaceIndex({Lbar, Sbar, Jbar, Tbar, Lbar % 2});
      ...
      // If the sector is diagonal, then we need to fill in lower triangle of
      // the sector because only the upper triangle was saved to file.
      if (bra_index == ket_index)
      {
        int nmax = block.cols() - 1;
        for (int n = 0; n <= nmax; ++n)
          for (int np = 0; np < n; ++np) { block(n, np) = block(np, n); }
      }
      ...
      if (get_adoint_sector)
      {
        ...
        // \braket{J'||V^{J_0}||J}=(-1)^{J'-J}
        // \hat{J}/\hat{J'}\braket{J||V^{J_0}||J'} SU_J(2) and SU_T(2).
        double conjugation_factor =
            ParitySign(Jbar - Jbarp + static_cast<int>(Tbar - Tbarp)) * (Hat(Jbar) * Hat(Tbar))
            / (Hat(Jbarp) * Hat(Tbarp));
  ...
  )
{
  ...
  for (size_t index_u3st = 0; index_u3st < sectors_u3st.size(); ++index_u3st)
  {
    ...
    const auto& spatial_subspace =sector_u3st.bra_subspace();
    const auto& spin_subspace = sector_u3st.ket_subspace();
         ...
         ++index_spin_state)
    {
      ...
      for (unsigned int Lbar = parity_bar; Lbar <= Nbar_max; Lbar += 2)
        for (unsigned int Lbarp = parity_bar; Lbarp <= Nbar_max; Lbarp += 2)
        {
          if (!am::AllowedTriangle(Lbar, L0, Lbarp))
            continue;

          basis::OperatorBlock<double> block_lst;
          const std::tuple<unsigned int, unsigned int, unsigned int> bra_label{
              Lbarp, Sbarp, Tbarp
            };
          const std::tuple<unsigned int, unsigned int, unsigned int> ket_label{
              Lbar, Sbar, Tbar
            };

          // Upcouple to LST
          block_lst = UpcoupleLST(
              {L0, S0, T0, J0}, bra_label, ket_label, sectors_lsjt, blocks
            );
          if (block_lst.rows() == 0)
            continue;
          ...
          for (size_t index_x0 = 0; index_x0 < spatial_subspace.size(); ++index_x0)
          {
                 ...
                 ++kappa0)
            {
              ...
              for (int index_spatial_state = 0;
                   index_spatial_state < x0_subspace.size();
                   ++index_spatial_state)
              {
                const auto& spatial_state =
                    x0_subspace.GetState(index_spatial_state);
                ...
                int n =
                    int(static_cast<int>(Nbar-Lbar) / 2);
                int np =
                    int(static_cast<int>(Nbarp-Lbarp) / 2);
                ...
                rme_array[array_index] +=
                    u3::W(
                        {Nbar, 0u}, 1, Lbar, x0, kappa0, L0, {Nbarp, 0u}, 1, Lbarp, 1
                      )
                    * (u3::dim(x0) * am::dim(Lbarp))
                    / (1. * u3::dim({Nbarp, 0u}) * am::dim(L0))
                    * ParitySign(n + np) * block_lst(np, n);
              }
            }
          }
        }
  ...
  )
{
  ...
  basis::ReadRelativeOperatorLSJT(
      input_filename, relative_space_lsjt, op_labels_lsjt, sectors_lsjt, blocks_lsjt, true
    );
  ...
  )
{
  ...
  u3shell::relative::OperatorSectors sectors_u3st = ConstructOperatorSectors(parameters);
