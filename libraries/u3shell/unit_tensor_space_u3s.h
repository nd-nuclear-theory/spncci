...
//*********************************************** Added by J.H. ******************************************
  ...
  typedef std::tuple<u3::SU3,HalfInt,unsigned int,unsigned int,unsigned int,unsigned int,u3::SU3,u3::SU3,int> TwoBodyDensitySubspaceLabels;
  ...
  // Labeling
  //
  // subspace labels: (x0,S0,N1,N2,N3,N4,xf,xi,rho0)
  //
  // state labels : (Sf,Si,Tz)
  //
  ////////////////////////////////////////////////////////////////
  ...
  // subspace
  ////////////////////////////////////////////////////////////////
  using TwoBodyDensityStateLabels = std::tuple<int,int,int>;

  class TwoBodyDensitySubspace
    : public basis::BaseSubspace<TwoBodyDensitySubspace,
        std::tuple<u3::SU3,HalfInt,unsigned int,unsigned int,unsigned int,unsigned int,u3::SU3,u3::SU3,int>,
        TwoBodyDensityState,
        std::tuple<int,int,int>>
    // SubspaceLabelsType (std::tuple): <x0,S0,N1,N2,N3,N4,xf,xi,rho0>
    //   x0 (SU3) : SU(3) label
    //   S0 (HalfInt) : spin
    ...
    //   rho0 (int) : outer multiplicity index of coupling xf x xi -> x0

    // StateLabelsType (std::tuple): <Sf,Si,Tz>
    //   Sf (int) : spin of coupled creation operators
    //   Si (int) : spin of coupled anihilation operators
    //   Tz (int) : +1 for proton TBD, -1 for neutron TBD, 0 for proton-neutron TBD
    {
  public:
    ...
    u3::SU3 xf() const {return std::get<6>(labels());}
    u3::SU3 xi() const {return std::get<7>(labels());}
    int rho0() const {return std::get<8>(labels());}
    int N0() const {return N1()+N2()-N3()-N4();}

    std::tuple<u3::SU3,HalfInt,unsigned int,unsigned int,unsigned int,unsigned int,u3::SU3,u3::SU3,int> Key() const {return labels();}
  ...
  TwoBodyDensityState(const SubspaceType& subspace, const typename SubspaceType::StateLabelsType& state_labels)
    ...
    u3::SU3 xf() const {return subspace().xf();}
    u3::SU3 xi() const {return subspace().xi();}
    int rho0() const {return subspace().rho0();}
    int N0() const {return N1()+N2()-N3()-N4();}

    // state label accessors
    int Sf() const {return std::get<0>(labels());}
    int Si() const {return std::get<1>(labels());}
    int Tz() const {return std::get<2>(labels());}
