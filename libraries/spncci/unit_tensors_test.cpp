...
int main(int argc, char **argv)
{
  ...
  u3::UnitSymmetryGenerator(N1b, Nmax,unit_sym_map);
  ...
  for (int i=0; i<lgi_pair_vector.size(); i++)
  	{
  		...
  		unit_tensor_rme_map[lgi_pair]=temp_unit_map;

  		// for (auto it=unit_tensor_rme_map.begin(); it !=unit_tensor_rme_map.end(); ++it)
  		// 	for (auto i=unit_tensor_rme_map[it->first].begin(); i !=unit_tensor_rme_map[it->first].end(); i++)
		  // 		{
		  // 			std::cout <<i->second<<std::endl;
		  // 		}
			//u3::TestFunction(Nmax, lgi_pair, unit_sym_map, unit_tensor_rme_map);
  		u3::UnitTensorMatrixGenerator(Nmax, lgi_pair, unit_sym_map, unit_tensor_rme_map);
