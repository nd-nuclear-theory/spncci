...
LGIGroups(const lgi::MultiplicityTaggedLGIVector& lgi_vector)
...
)
{
  ...
  for(int sp3r_space_index=0; sp3r_space_index<spatial_recurrence_space.size(); ++sp3r_space_index)
    {      
      ...
      // std::cout<<sigma_ket.Str()<<"  "<<sigma_bra.Str()<<std::endl;
      ...
      for(const auto&[irrep_family_index_bra,lgi_bra] : lgi_groups.at(sigma_bra))
        for(const auto&[irrep_family_index_ket,lgi_ket] : lgi_groups.at(sigma_ket))
          {
            // std::cout<<"irrep families: "<<irrep_family_index_ket
            // <<"  "<<irrep_family_index_bra<<std::endl;
            assert(lgi_bra==lgi_vector[irrep_family_index_bra].irrep);
            assert(lgi_ket==lgi_vector[irrep_family_index_ket].irrep);
            ...
            // std::cout<<spin_subspace_index<<std::endl;
            ...
            // std::cout<<"Populating seed matrix " <<unit_tensor_seed_blocks.size()<<std::endl;
            for(int t=0; t<lgi_unit_tensors.size(); ++t)
              {
                //Extract unit tensor labels 
                // std::cout<<lgi_unit_tensors[t].Str()<<std::endl;
                  ...
                  for (int g_bra=0; g_bra<degeneracy_bra; ++g_bra)
                    {
                      ...
                      // g=gamma*gammap_max + gammap (hopefully)
                      // Note: degeneracy is 1 based

                      int target_index_col 
                        = lgi_spin_space.GetSubspaceOffset(spin_space_index)
                          +spin_space.GetSubspaceOffset(spin_subspace_index,g)
                          + spin_operator_index;

                      // std::cout<<target_index_row<<" x "<<target_index_col<<std::endl;
                      // std::cout<<seed_blocks[sp3r_space_index].rows()<<" x "
                      // <<seed_blocks[sp3r_space_index].cols()<<std::endl;
                      // std::cout<<g_bra<<" x "<<g_ket<<std::endl;
                      // std::cout<<block.rows()<<" x "<<block.cols()<<std::endl;
                      seed_blocks[sp3r_space_index](target_index_row,target_index_col)
                        = block(g_bra,g_ket);
                      // Increment g
                      g++;
                    }
              }
            // std::cout<<"end unit tensors"<<std::endl;
          }
